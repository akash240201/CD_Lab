...
void parse(char *str, vector<vector<string>> &tokens)
{
    int left = 0, right = 0;
    int len = strlen(str);
    while (right <= len && left <= right)
    {
        ...
        if (isPunctuator(str[right]) == true && left == right)
        {
            ...
            {
                string token = to_string(str[right]);
...
int main()
{
    ...
    vector<vector<string>> tokens{operators, keywords, numbers, identifiers};
    parse(c, tokens);
    return 0;
}
